...
**Feature ids:** `agents`, `mcp-tools-agents`, `ytdlp-updater` ·
**Dashboard:** `/agents` · **SSE topic:** `agents` · **Priority 13**

An assistant hands a goal to a **copy of itself that works in the background**:
the same persona, the same toolset as the chat turn that started it, plus a
**real browser** — search, navigate, click, type, scroll, read, inspect network
traffic, screenshot, download. The run acts as the assistant in the chat it came
from, posts its final report there when done, and can record what it finds
through the same tools the assistant has (user decision, 2026-09-08).

It grew out of the browser agent, which spawned a browser-only helper: its own
prompt, a static tool list, no turn context, no persona, and a text report the
runner posted. Such a run could read a page but record nothing anywhere, so
"look this up and put it in the collection" was impossible inside a run. The
one background tool is now an explicit delegation, `start_agent`, replacing
`browse_web` (no alias — user decision, 2026-09-08).

This is still the bot's **only** web-facing tool (user decision, 2026-07-26).
The earlier `search_web` (Tavily snippets) and `read_web_page` (one-shot page
read) MCP tools are **gone**: a real browser does both jobs better, and offering
the model two weaker alternatives only split its choice — every phrasing became
a fight over which tool should win. Searching happens *inside* a run, on live
search engines, with the Tavily API kept as a last-resort fallback there.

## Two layers, deliberately separated

| Layer | What the main chat model sees |
| --- | --- |
| `start_agent` (MCP tool) | One dispatch tool. Enqueue a goal, get an ack, move on |
| The run's own toolset | **Nothing.** The browser primitives are plain OpenAI tool definitions for the agent's own loop, never offered to the chat model; the assistant's tools the run also holds are the ones the chat model already has |

The chat model does **not** do the work itself (recorded decision: background
run, not inline). A run is many rounds of latency; holding a chat turn open for
it would stall the chat.

## `start_agent`

Input: `goal` — 4–4000 chars, self-contained, every link and id copied
character-for-character; `context` — optional, ≤4000 chars, the facts from the
conversation the agent needs and cannot see (names, ids, what was decided, how
the result should look), since a run sees no transcript; `quiet` — optional,
for batch work nobody is waiting on. Output: `{ ok, runId? }`.
...
7. The job has to **act on what it finds** — look something up and record it, fill
   in missing details, work through a list — or would take many steps or minutes.
   (The one case a browsing tool never had.)
...
And the DO-NOTs: casual chat, an opinion, a stable fact the model already knows
well and was not asked to verify, and anything the turn can finish right now with
its own tools. Since there is no cheaper web tool left, the description no longer
has to argue about when to prefer a plain search — the earlier carve-outs for "a
commodity live value" and "a general lookup" are gone, and those requests now go
to a run too. The description names no other tool (the tool-description rule).
...
from anything the model says: `start_agent` reads the bound turn context's
...
The whole turn binding rides on the run row — assistant, chat, thread, sender,
both owner flags, the correlation, the message's verbatim URLs — stamped by
`start_agent` at enqueue. The runner rebuilds it (`server/run-binding.ts`,
`runTurnBinding`) and wraps the agent loop in `runWithToolContext`, exactly as a
reply turn or a task fire does, so every tool the run calls reads and writes as
that turn would: memory files under the right person, a task lands in the right
chat for the right assistant, a hosted tool receives the same `_meta` binding.
Two facts about the binding are the run's own:

| Fact | Value | Why |
| --- | --- | --- |
| `deliveryKind` | `send` | A run speaks unprompted, like a timed fire; nothing triggered it that it could reply to |
| `silentDelivery` | `true` | Anything the agent sends mid-run goes out **without a notification ping**; the final report the runner posts afterwards is the one that pings |

The silent flag travels to hosted delivery tools in the call's `_meta`
(`silentDelivery`, added only when set, so every other turn's binding is
byte-for-byte what it was) and the SDK's delivery tools pass it to the platform
send. The web chat has no pings, so its in-process tool has nothing to do with it.

**What the run holds** (`server/agent.ts`, `composeAgentTools`): the browser
primitives first, then `getToolset({ delivery: "send" })` for the run's assistant
— every in-process tool and every tool connection in scope, the chat's
`send_message` included — minus `start_agent` itself (a run does not spawn
runs) and minus any name the browser set already owns. Dispatch follows
ownership: a browser name goes to the browser dispatcher, anything else to the
assistant's toolset. Each non-browser call is recorded on the run's activity
feed like a browser action (tool name, ok, one-line summary); the call's own
trace is the registry's, under the tool's feature, correlated to the run's
starting turn.

**The prompt** puts the persona first — the assistant's identity line and
persona block, as a chat turn composes them — then the working method: the
browser rules (navigate first when given a URL, refs from the latest page
state, no guessed URLs), the rule that binds the goal to tool effects ("the goal
is not done until what it asks for has actually been done through a tool, never
by describing it in the report"), the rule that reserves the final reply as the
report ("do not also send it with a message tool; send a message mid-run only for
a note worth interrupting for"), the quiet line when the run is quiet, and the
download rules gated on owner rights. It names no tools beyond the browser
protocol it explains. The user turn is the goal, then the context the starting
turn gathered, then the verbatim URL list.

**Quiet runs.** A run started with `quiet: true` — batch work from a fire, say
"fill the gaps in the collection" — stores its report on the run row and posts
it to the chat **only when the goal failed** (the outcome check's verdict) **or
a file was downloaded**: a failure is the one thing the chat must hear, and a
...
— three rounds that each introduce no new tool call — and its forced tools-free
final round then salvages a report from whatever was gathered.
...
- One run at a time is also what makes a batch loop safe: a second run started
  while the first still works simply starts afterwards, re-reads the state, and
...
- **A quiet run posts nothing** unless the goal failed or a file was staged; its
  report is stored on the row and shown on the dashboard, and the trace records
...
- The chat model's own "on it" reply — sent when `start_agent` enqueues the run —
...
| Table | Notes |
| --- | --- |
| `agent_runs` | The queue and the record. The turn binding: `chat_ref`, `thread_id`, `created_by_user_ref`, `assistant_id`, `is_owner`, `sender_is_owner`, `authority_is_owner`, `correlation_id`, `restricted`, `source_urls`. The work: `goal`, `context`, `quiet`. The outcome: `status`, `report`, `error`, `steps`, `activity` (jsonb step feed), `downloads` (jsonb), `trace_id` |
...
| Setting | Effect |
| --- | --- |
| Agent role (`agentBackendId`/`agentModel`, chat backend + model by default) | The model every run thinks on. It now holds the assistant's whole toolset next to the browser, so pick one that copes with a large tool list and long loops. Without a resolvable connection a run settles as a failure |
| Owner rights (the assistant's owning account + admins, stamped on the turn as `sender.isOwner`) | Only runs carrying owner rights (the sender's own, or lent by a standing rule) may download — read from `agent_runs.is_owner` / `restricted`, stamped at enqueue |
...
| `/agents` | A warning banner above the runs list |
...
Feature `agents`, `relatedIdsKey` `agent_runs`, and the run row carries
the `trace_id` for drill-down. The run trace records the binding it acts under
(assistant, whether a persona was composed, the assistant tools offered, quiet),
every browser action and every assistant tool call as events, and a withheld
report on a quiet run. Its LLM rounds carry the call kinds `agent-turn` and
`agent-report`; the retired `browser-agent-turn` / `browser-agent-report` ids
stay labelled in Analytics so month files from before the rename still
categorize. The `start_agent` dispatch call is separately traced under
`mcp-tools-agents`; every tool the run calls traces under that tool's own
feature, correlated to the run's starting turn, so `/debug?flow=` walks the
whole story from the chat message to the run's last tool call.
...
`server/agent.test.ts` (the conversation compaction; the toolset composition —
browser first, the assistant's next, never `start_agent`, dispatch by
ownership; the prompt — persona first, the tool-effect and no-double-send
...
Integration: `server/agents.integration.test.ts` (the queue lifecycle and the
binding round-trip), and the silent flag on hosted tools in
`features/tool-connections/server/tool-connections.integration.test.ts`.
Live, and marked as such: `server/browse-live.integration.test.ts`,

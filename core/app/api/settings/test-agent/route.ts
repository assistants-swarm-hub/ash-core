...
import { testAgent } from "@/features/settings/server/service";
import { defineRoute, ok, parseJson } from "@/server/http";

/**
 * Probe the agent role by running one real tool round — a model listing
...
export const POST = defineRoute(async ({ request }) => {
  ...
  return ok(await testAgent(input, { kind: "dashboard" }));
});

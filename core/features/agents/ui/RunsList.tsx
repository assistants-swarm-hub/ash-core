  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        const body = (await res.json()) as { data: AgentRunDetail };
    ...
    <div className="space-y-4 bg-surface-2/40 px-4 py-4">
      ...
      {view.context ? (
        <div>
          ...
          <p className="mt-1 whitespace-pre-wrap text-sm text-foreground">{view.context}</p>
        </div>
      ) : null}
        ...
        <div>
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
            {screenshotSeqs.map((seq) => (
              <a
                key={seq}
                href={`/api/agents/${run.id}/screenshot/${seq}`}
                target="_blank"
                rel="noreferrer"
              ...
              >
                ...
                <img
                  src={`/api/agents/${run.id}/screenshot/${seq}`}
...
/**
 * Live-refreshing table of agent runs. Each row expands to show the
...
export function RunsList({
  ...
  runs: AgentRun[];
  /** Assistant display names by id, for the "as …" line of each run. */
                ...
                <Fragment key={run.id}>
                  ...
                  >
                    ...
                    <TableCell valign="middle" className="max-w-md">
                      ...
                      <p className="mt-0.5 text-xs text-muted">{runActor(run, assistantNames)}</p>
                    </TableCell>

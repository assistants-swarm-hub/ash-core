...
import type { AgentRunLiveState } from "../types";
...
const STORE_KEY = Symbol.for("assistants-swarm-hub.agents.live-state");

function store(): Map<string, AgentRunLiveState> {
  const g = globalThis as typeof globalThis & { [STORE_KEY]?: Map<string, AgentRunLiveState> };
...
export function getLiveState(runId: string): AgentRunLiveState | null {

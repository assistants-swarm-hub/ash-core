...
describe("rememberUser", () => {
  it("captures a user on first message and refreshes the profile without touching aliases", async () => {
    await rememberUser("acme",
      { userId: "1", username: "ann", firstName: "Ann", lastName: null },
      ctx.db,
    );
    await updateAliases("acme:user:1", { aliases: ["Boss"] }, trigger, ctx.db);
    ...
    const user = await getKnownUser(ctx.db, "acme", "1");
  ...
  it("traces the capture only when it adds or changes data, not on re-sightings", async () => {
    ...
    await rememberUser("acme", profile, ctx.db);
    // Identical re-sighting → no new trace.
    await rememberUser("acme", profile, ctx.db);
    // Changed profile → an update-profile trace.
    await rememberUser("acme", { ...profile, lastName: "Vine" }, ctx.db);
...
describe("listUsers", () => {
  it("returns users most-recently-seen first", async () => {
    await upsertKnownUser(ctx.db, "acme", { userId: "1", username: "first", firstName: null, lastName: null });
    await upsertKnownUser(ctx.db, "acme", { userId: "2", username: "second", firstName: null, lastName: null });

    const users = await listUsers("acme", ctx.db);
...
describe("updateLanguage / getUserLanguage", () => {
  it("sets and clears the DM language, recording a trace and surviving profile upserts", async () => {
    await upsertKnownUser(ctx.db, "acme", { userId: "1", username: "ann", firstName: "Ann", lastName: null });

    // Unset → null (the runtime falls back to the default).
    expect(await getUserLanguage("acme", "1", ctx.db)).toBeNull();

    const set = await updateLanguage("acme:user:1", { language: "Ukrainian" }, trigger, ctx.db);
    expect(set.language).toBe("Ukrainian");
    expect(await getUserLanguage("acme", "1", ctx.db)).toBe("Ukrainian");

    // A later message must not wipe the operator-configured language.
    await rememberUser("acme", { userId: "1", username: "ann2", firstName: "Ann", lastName: null }, ctx.db);
    expect(await getUserLanguage("acme", "1", ctx.db)).toBe("Ukrainian");

    const cleared = await updateLanguage("acme:user:1", { language: null }, trigger, ctx.db);
  ...
  it("fails for an unknown user and records an error trace", async () => {
    await expect(
      updateLanguage("acme:user:404", { language: "Ukrainian" }, trigger, ctx.db),
    ).rejects.toThrow(/unknown user/i);
  });

  it("returns null language for an unknown user", async () => {
    expect(await getUserLanguage("acme", "404", ctx.db)).toBeNull();
  });
});
...
describe("updateAliases", () => {
  it("replaces the alias list and records a trace", async () => {
    ...
    const updated = await updateAliases("acme:user:1", { aliases: ["Boss", "Chief"] }, trigger, ctx.db);
  ...
  it("fails for an unknown user and records an error trace", async () => {
    await expect(updateAliases("acme:user:404", { aliases: [] }, trigger, ctx.db)).rejects.toThrow(
      /unknown user/i,
    );
    const { traces } = await listTraces({ feature: "known-users" });
...
describe("addAliasByReference", () => {
  ...
  async function seedParticipant(
    ...
    await upsertKnownGroup(ctx.db, "acme", { chatId, title: "Fixture Group", type: "supergroup" });
    await recordGroupMembership(ctx.db, "acme", chatId, profile.userId);
  }
  ...
  it("resolves a participant by name and appends the new alias, tracing the change", async () => {
    ...
    const result = await addAliasByReference(
      { source: "acme", chatId: CHAT, reference: "alice", aliases: ["Ali"] },
      { kind: "transport", actor: CHAT },
      ctx.db,
    );
    expect(result).toMatchObject({ status: "updated", added: ["Ali"] });
    expect((await getKnownUser(ctx.db, "acme", "1"))?.aliases).toEqual(["Ali"]);
  ...
  it("returns not_found for a name that no participant matches", async () => {
    ...
    const result = await addAliasByReference(
      { source: "acme", chatId: CHAT, reference: "charlie", aliases: ["C"] },
      { kind: "transport", actor: CHAT },
      ctx.db,
    );
  ...
  it("is a no-op when the alias is already implied by the user's identity", async () => {
    await seedParticipant({ userId: "1", username: "alice", firstName: "Alice", lastName: null });
    const result = await addAliasByReference(
      { source: "acme", chatId: CHAT, reference: "alice", aliases: ["Alice", "@alice"] },
      { kind: "transport", actor: CHAT },
      ctx.db,
    );
    expect(result.status).toBe("noop");
    expect((await getKnownUser(ctx.db, "acme", "1"))?.aliases).toEqual([]);
  });
